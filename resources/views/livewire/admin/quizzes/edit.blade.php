...
<div>
    <div class="mt-10 text-center">
        <h2 class="text-fuchsia-800 font-bold text-center">Sınavını Güncelle</h2>
    </div>
    <div class="mt-20">
        <form x-ref="form" wire:submit.prevent="submit" class="max-w-sm mx-auto">
            <div class="mb-5">
                <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Sınav Başlıgı</label>
                <input wire:model="form.title" type="title" id="title" name="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#EFF1DB] focus:border-[#EFF1DB] block w-full p-2.5 dark:border-gray-600 dark:placeholderbg-gray-400 dark:text-white ">
            ...
            <div>
                ...
                <div class="relative max-w-sm mt-5">
                    ...
                    <select wire:model="form.status"  name="status" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-[#EFF1DB] focus:border-[#EFF1DB] focus:outline-none focus:ring-1  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-[#EFF1DB]">
                        <option @if($form->quiz->status == 'publish') selected @endif value="publish">Aktif</option>
                        <option @if($form->quiz->status == 'passive') selected @endif value="passive">Pasif</option>
                        <option @if($form->quiz->status == 'draft') selected @endif value="draft">Taslak</option>
                    </select>
                    <div class="text-red-500 mt-2">
